...
def validate_relative_time(time_str):
    ...
    Parameters:
    - time_str (str): Relative time string (example"in 3 hours").

    Returns:
    - (bool, int, str): ligne with validity (booléen), quantity (int), and unit (str).
    """
    try:
...
def setup(bot):
    ...
    async def set_timezone(ctx, timezone: str):
        ...
        try:
            ...
            user_data["timezone"] = timezone  # update user's timezone
    ...
    async def schedule(ctx, title: str, time_str: str):
        ...
        try:
            ...
            # parsing time (absolute or relative)
            if time_str.startswith("in"):
